...
export class NeedPayError extends Error {
  constructor(public wallet: Wallet) {
    super(
      wallet.credits <= 0
        ? "这一问需要扣 1 次。你目前次数不够了，去「充值」买次数或开通月租后再问我。"
        : "先充值后再问这一盘。",
    );
...
function labelOf(plan: PlanKind, lifetime: boolean, until: string | null, credits: number) {
  ...
  if (plan === "monthly" && until && new Date(until).getTime() > Date.now()) {
    return `月租至 ${untilDay(until)}`;
  }
  return `按次 ${credits} 次`;
}
...
export function readWallet(row: {
  ...
  const monthlyOn =
    plan === "monthly" && Boolean(planUntil) && new Date(planUntil as string).getTime() > Date.now();
...
export async function consumeCast(userId: string, sessionId: string): Promise<Wallet> {
  ...
  if (wallet.unlimited) {
    ...
    await sql`
      ...
      values (${userId}, ${sessionId}, ${"cast"}, ${0}, ${wallet.lifetimeFree ? "永久免费" : "月租"})
    `;
    return wallet;
  }
...
async function applyPaidSku(userId: string, sku: PaySku) {
  ...
  if (item.plan === "monthly") {
    return grantPlan(userId, { plan: "monthly" }, "月租到账");
  }
...
export const getWallet = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const payments = await sql<{
      ...
      select id, channel, sku, amount_fen, credits, status, created_at, paid_at
...
export const createPayment = createServerFn({ method: "POST" })
  ...
  .validator(
    z.object({
      sku: z.enum(["credits_1", "credits_10", "credits_30", "credits_88", "monthly"]),
  ...
  .handler(async ({ context, data }) => {
    ...
    const sql = await getSql();
    await sql`
      insert into payments (id, user_id, channel, sku, amount_fen, credits, status)
      values (
        ${id}, ${context.userId}, ${data.channel}, ${data.sku},
        ${item.amountYuan * 100}, ${item.credits}, ${"pending"}
      )
    `;
    return {
      id,
...
export const confirmPayment = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    if (row.status === "paid") return { ok: true as const, wallet: await loadWallet(context.userId) };
    await sql`
      update payments set status = ${"paid"}, paid_at = now()
      where id = ${row.id} and status = ${"pending"}
    `;
    const wallet = await applyPaidSku(context.userId, row.sku as PaySku);
